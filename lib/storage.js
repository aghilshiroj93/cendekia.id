...
const DEFAULT_INFO = {
  ...
  phone: '6281234567890',
  email: 'info@cendekia.id',
  hours: 'Senin - Sabtu: 09:00 - 18:00'
};

export const getInfo = () => {
  if (typeof window === 'undefined') return DEFAULT_INFO;
  const data = localStorage.getItem('cendekia_info');
  return data ? JSON.parse(data) : DEFAULT_INFO;
};

export const saveInfo = (info) => {
  localStorage.setItem('cendekia_info', JSON.stringify(info));
};

const DEFAULT_TESTIMONIALS = [
  { id: '1', name: 'Budi Santoso', role: 'Mahasiswa Akhir', content: 'Sangat terbantu dengan jasa pengerjaan jurnalnya. Cepat dan hasilnya memuaskan!' },
  { id: '2', name: 'Siska Putri', role: 'Owner E-Commerce', content: 'Website yang dibuat sangat rapi dan responsif. Penjualan saya meningkat setelah menggunakan jasa CENDEKIA.ID.' },
  { id: '3', name: 'Dr. Ahmad', role: 'Dosen Pengampu', content: 'Analisis data science yang diberikan sangat akurat dan mendalam. Sangat profesional!' }
];

export const getTestimonials = () => {
  if (typeof window === 'undefined') return DEFAULT_TESTIMONIALS;
  const data = localStorage.getItem('cendekia_testimonials');
  return data ? JSON.parse(data) : DEFAULT_TESTIMONIALS;
};

export const saveTestimonials = (testimonials) => {
  localStorage.setItem('cendekia_testimonials', JSON.stringify(testimonials));
};
